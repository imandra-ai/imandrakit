...
let run_test (c : count) (t : Test.t) : unit =
  try
    ...
    | v ->
      if verbose then Fmt.printf "  decoded into %a@." Cbor.pp_diagnostic v;
      ...
      | None ->
        ...
        if
          ...
          | _ -> true
        then (
          let hex' = Cbor.encode v |> CCString.to_hex in
          if hex' <> t.hex then (
            Fmt.printf
        ...
        (match t.expect with
        | Test.Diagnostic s ->
          let s' = Cbor.show_diagnostic v in
          (* adjust display *)
          let s' =
            match s' with
            | "inf" -> "Infinity"
        ...
        | Test.Decoded j ->
          let rec compare_cj (v : V.t) (j : json) =
            match v, j with
            | Null, `Null -> true
            ...
            | Dict d, `Assoc l2 ->
              List.for_all
                (fun (k, v) ->
                  try compare_cj (Str_map.find k d) v with Not_found -> false)
                l2
            | Int i, `Int j -> i = Int64.of_int j
            | Str s1, `String s2 -> s1 = s2
            | List l1, `List l2 ->
            ...
            | _, `Intlit "-18446744073709551617" ->
              ...
              true
            | _ ->
              Fmt.printf "  TODO: compare %a with %a@." V.pp v J.pp j;
              true
          in

          let ok = compare_cj v j in
