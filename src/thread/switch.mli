...
(** New switch.
   ...
   In other words, this switch's lifetime is a subset of the parent's lifetime *)

val with_on_turn_off : t -> (unit -> unit) -> (unit -> 'a) -> 'a
(** [with_on_turn_off sw cb f] calls [f()]. Within the context where [f()]
  is running, [sw] turning off will trigger [cb()].
  Once [f()] has terminated, [cb] is disabled and will not be called. *)

val on_turn_off : t -> (unit -> unit) -> unit
(** [on_turn_off sw f] will call [f()] when [sw] is turned off.
    If [sw] is already off then [f()] is called immediately.
