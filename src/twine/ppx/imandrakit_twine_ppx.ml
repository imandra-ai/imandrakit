...
let rec immediate_expr_of_ty (e : expression) ~(ty : core_type) : expression =
  ...
  | [%type: _ Imandrakit_twine.offset_for] ->
    [%expr Imandrakit_twine.Immediate.ref_for [%e e]]
...
let rec decode_expr_of_ty (e : expression) ~(ty : core_type) : expression =
  ...
  | [%type: [%t? _] Imandrakit_twine.offset_for] ->
    by_full_dec [%expr Imandrakit_twine.Decode.ref_for]
  | [%type: [%t? ty_arg0] option] ->
    [%expr
      ...
      let fail =
        ...
        and rhs =
          let err =
            A.Exp.constant
              (A.Const.string
              @@ spf "expected value of type %s (defined in at %%s), got %%s"
                   decl.ptype_name.txt)
          in
          [%expr
            Imandrakit_twine.Decode.failf [%e err] __LOC__
              (Imandrakit_twine.Decode.Value.show e)]
        in
