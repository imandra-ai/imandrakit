...
let rec hasher_name_of_lid (lid : Longident.t) : Longident.t =
  match lid with
  | Longident.Lident name -> Longident.Lident (hasher_name_of_ty_name name)
  | Longident.Lapply (a, b) -> Longident.Lapply (a, hasher_name_of_lid b)
  | Longident.Ldot (m, a) -> Longident.Ldot (m, hasher_name_of_ty_name a)
...
let attr_hasher =
  Attribute.declare "hash.hasher" Attribute.Context.core_type
    Ast_pattern.(single_expr_payload __)
    (fun x -> x)

(* attribute to skip hashing *)
let attr_no_hash =
  Attribute.declare "hash.nohash" Attribute.Context.core_type
...
let apply_hasher ~loc e_hasher e : expression =
  [%expr
    let e = [%e e] in
    let h = [%e e_hasher] in
    h.Imandrakit_mhash.hash_into algo ctx e]
...
let rec hash_expr_of_type (e : expression) ~(ty : core_type) : expression =
  ...
  let by_hasher h = apply_hasher ~loc h e in
  (* just apply a hasher *)
  match ty with
  ...
  | _ when is_some_ (Attribute.get ~mark_as_seen:false attr_hasher ty) ->
    (* custom hasher *)
    let hasher =
      match Attribute.get ~mark_as_seen:true attr_hasher ty with
      | None -> assert false
      | Some h -> h
    in
    apply_hasher ~loc hasher e
  | [%type: int] -> by_hasher [%expr Imandrakit_mhash.int]
  | [%type: int32] -> by_hasher [%expr Imandrakit_mhash.int32]
  | [%type: int64] -> by_hasher [%expr Imandrakit_mhash.int64]
  | [%type: nativeint] -> by_hasher [%expr Imandrakit_mhash.nativeint]
  | [%type: string] -> by_hasher [%expr Imandrakit_mhash.string]
  | [%type: bytes] -> by_hasher [%expr Imandrakit_mhash.bytes]
  | [%type: bool] -> by_hasher [%expr Imandrakit_mhash.bool]
  | [%type: char] -> by_hasher [%expr Imandrakit_mhash.char]
  | [%type: unit] -> by_hasher [%expr Imandrakit_mhash.trivial]
  | [%type: float] ->
    (* hash float as its bitwise representation (an int64) *)
    apply_hasher ~loc [%expr Imandrakit_mhash.int64]
      [%expr Int64.bits_of_float [%e e]]
  | [%type: [%t? ty_arg0] option] ->
    [%expr
      match [%e e] with
      | None -> algo.Imandrakit_mhash.bool ctx false
      | Some x ->
        algo.Imandrakit_mhash.bool ctx true;
        [%e hash_expr_of_type [%expr x] ~ty:ty_arg0]]
  | [%type: [%t? ty_arg0] list] ->
    [%expr
      List.iter (fun x -> [%e hash_expr_of_type [%expr x] ~ty:ty_arg0]) [%e e]]
  | [%type: [%t? ty_arg0] array] ->
    [%expr
      Array.iter (fun x -> [%e hash_expr_of_type [%expr x] ~ty:ty_arg0]) [%e e]]
  ...
  | { ptyp_desc = Ptyp_alias (ty, _); _ } -> hash_expr_of_type e ~ty
...
let hash_expr_of_tydecl (decl : type_declaration) : expression =
  ...
  let body =
    ...
    | Ptype_abstract ->
      ...
      | None -> [%expr [%error "cannot derive hash for abstract type"]])
    | Ptype_open -> [%expr [%error "cannot derive hash for open type"]]
    | Ptype_variant cstors ->
      let hash_cstor (i : int) { pcd_args; pcd_name = cname; pcd_loc = loc; _ }
          : case =
        (* hash the constructor index *)
        let hash_cstor_idx =
          [%expr
            algo.Imandrakit_mhash.int ctx [%e A.Exp.constant (A.Const.int i)]]
        in
    ...
    | Ptype_record labels ->
      ...
      e
  in
  [%expr { Imandrakit_mhash.hash_into = (fun algo ctx self -> [%e body]) }]
...
let param_names ty =
  ...
  |> List.map (fun (ty, _) ->
         ...
         | Ptyp_var a -> a
         | Ptyp_any -> error_gen ~loc "Cannot derive hasher for implicit param"
...
let generate_impl_ (rec_flag, type_declarations) =
  ...
  let fun_poly_hashers ~loc ty body =
    mk_lambda ~loc (List.map hasher_name_poly_var @@ param_names ty) body
  in
    ...
    List.map
      (fun ty ->
        ...
        let rhs =
          ...
          @@ [%expr
               fun ~algo x -> Imandrakit_mhash.hash ~algo ~hash:[%e hasher] x]
        in
    ...
    List.map
      (fun ty ->
        ...
        let poly_hashers =
          param_names ty
          |> List.map (fun n ->
                 [%type: [%t A.Typ.var n] Imandrakit_mhash.hasher])
        in
        ...
        let decl_hasher =
          let ty =
            ...
            @@ [%type: [%t tye] Imandrakit_mhash.hasher]
          in
        ...
        let decl_hash =
          let ty =
            mk_arrow ~loc poly_hashers
            @@ [%type:
                 algo:(_, 'out) Imandrakit_mhash.hash_algo -> [%t tye] -> 'out]
          in
...
let generate_intf ~ctxt:_ (_rec_flag, type_declarations) =
  ...
  with Error_gen (loc, msg) ->
    (* emit an error in generated code *)
    let s =
      [%sigi: val _bad_hash : [%error [%e A.Exp.constant (A.Const.string msg)]]]
    in
    [ s ]
...
let myderiver =
  Deriving.add "mhash" ~sig_type_decl:intf_generator
